...
def dataset_handle(name, filelist, result, callback, bs, pindex, freearr, arrimage, arrlabel, zfile):
    ...
    nparrimage = np.frombuffer(
        arrimage.get_obj(), np.float32).reshape(10, (int)(len(arrimage)/10))
    nparrlabel = np.frombuffer(
        arrlabel.get_obj(), np.float32).reshape(10, (int)(len(arrlabel)/10))
    while True:
...
class ImageDataset(object):
    ...
    def __init__(self, imageroot, callback, imagelistfile=None, bs=1, shuffle=False,
        ...
        self.nparrimage = np.frombuffer(self.arrimage.get_obj(), np.float32).reshape(
            10, (int)(len(self.arrimage)/10))
        self.nparrlabel = np.frombuffer(self.arrlabel.get_obj(), np.float32).reshape(
            ...
            if os.path.isdir(imageroot):
                ...
                if '.zip:' in imageroot:
                    ...
                    if zipfilepath in ImageDataset.zipcache:
                        ...
                        self.zfile = zipfile.ZipFile(zipfilepath, mode='r')
