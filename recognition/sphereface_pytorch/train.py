...
parser.add_argument(
    '--dataset', default='../../dataset/face/casia/casia.zip', type=str)
...
def dataset_load(name, filename, pindex, cacheobj, zfile):
    ...
    # jpg = zfile.open(nameinzip)
    # data = np.frombuffer(jpg.read(), np.uint8)
...
for i in range(0, 19):
    net.load_state_dict(torch.load('sphere20a_{}.pth'.format(i)))
    print('pth{} loaded'.format(i))
net.cuda()
...
optimizer = optim.SGD(net.parameters(), lr=args.lr,
                      momentum=0.9, weight_decay=5e-4)  # added
